...
import { MotionPreset, StylePreset, GlyphItem } from '../types';
...
export const LeftLibrary: React.FC<LeftLibraryProps> = ({
  ...
  // Mini thumbnail rendering complete 17-glyph vector mark
  const renderThumbnailMark = (fillPrimary = '#ffffff', fillMedia = '#ff4e2e', stroke = 'none', strokeW = '0px') => (
    <svg width="28" height="28" viewBox="0 0 25 26" fill="none" className="block overflow-visible drop-shadow">
      <g>
  ...
  return (
    <aside className="w-[300px] flex-shrink-0 bg-[#0e1117] border-r border-[#1f2430] flex flex-col z-30 select-none">
      {/* Tab Switcher */}
      <div className="flex bg-[#0a0c10] border-b border-[#1f2430] p-1 gap-1">
        <button
      ...
      {/* Search Input */}
      <div className="p-2.5 border-b border-[#1f2430] flex flex-col gap-2">
              ...
              return (
                <div
                  ...
                  className={`group relative bg-[#131620] border rounded-lg p-2.5 cursor-pointer flex flex-col gap-2 transition-all ${
                    isSelected
                      ? 'border-[#ff4e2e] bg-[#ff4e2e]/[0.06] shadow-lg shadow-[#ff4e2e]/10'
                      : 'border-[#202534] hover:border-slate-500 hover:bg-[#181c28]'
                  }`}
                >
                  {/* Card Visual Stage Thumbnail */}
                  <div className="h-16 bg-[#07080c] border border-white/5 rounded flex items-center justify-center overflow-hidden relative">
                    <div className="group-hover:scale-105 transition-transform duration-300">
                      {renderThumbnailMark()}
                    </div>
                    {isSelected && (
                      <div className="absolute top-1 right-1 w-3.5 h-3.5 bg-[#ff4e2e] rounded-full flex items-center justify-center shadow">
                  ...
                  <div className="flex flex-col">
                    ...
                    <div className="flex items-center justify-between text-[9px] font-mono text-slate-400 mt-0.5">
                      <span className="text-[#ff4e2e] font-medium">{motion.badge}</span>
            ...
            {filteredStyles.map(style => {
              ...
              return (
                <div
                  ...
                  className={`group relative bg-[#131620] border rounded-lg p-2.5 cursor-pointer flex flex-col gap-2 transition-all ${
                    isSelected
                      ? 'border-[#38bdf8] bg-[#38bdf8]/[0.06] shadow-lg shadow-[#38bdf8]/10'
                      : 'border-[#202534] hover:border-slate-500 hover:bg-[#181c28]'
                  }`}
                >
                  {/* Style Preview Stage */}
                  <div
                    className="h-16 rounded border border-white/5 flex items-center justify-center overflow-hidden relative"
                  ...
                  >
                    ...
                    {isSelected && (
                      <div className="absolute top-1 right-1 w-3.5 h-3.5 bg-[#38bdf8] rounded-full flex items-center justify-center shadow">
        ...
        {activeTab === 'glyphs' && (
          <div className="grid grid-cols-3 gap-2">
            {filteredGlyphs.map(glyph => (
              <div
                key={glyph.id}
                className="bg-[#131620] border border-[#202534] hover:border-slate-500 rounded-lg p-2 flex flex-col items-center gap-1.5 transition-all"
              >
